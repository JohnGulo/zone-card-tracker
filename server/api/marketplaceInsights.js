// server/api/marketplaceInsights.js
...
// In-memory cache (same pattern as ebaySearch.js)
const cache = new Map();
...
router.get('/marketplace-insights', async (req, res) => {
  ...
  try {
    ...
    const endpoint = `https://api.ebay.com/buy/marketplace_insights/v1/item_sales/search?q=${encodeURIComponent(query)}&filter=sold_status:{SOLD}&limit=25`;
    ...
    console.log(`✅ [LIVE FETCH] ${itemSales.length} items for Marketplace Insights: ${query}`);
